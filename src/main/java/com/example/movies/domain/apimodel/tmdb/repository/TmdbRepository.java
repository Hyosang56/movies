package com.example.movies.domain.apimodel.tmdb.repository;
...
public class TmdbRepository {
    private static final String TMDB_API_KEY = "4c6aa989109ae6427ad0dbcc1023e53a";
    ...
    public List<TmdbMovie> getPopularList() {
        ...
        for (int i = 1; i < 11; i++) {
            ...
            ResponseEntity<TmdbMovie> response = restTemplate.getForEntity(
                    apiUrl, TmdbMovie.class, i); //응답은 TmdbMovie 클래스 형식으로 받아온다.

            //응답이 성공일 경우
            if(response.getStatusCode().is2xxSuccessful()) {
                //응답에서 가져온 TmdbMovie 객체를 tmdbMovieList에 추가
